...
public class AuthController {
    ...
    public Result refreshToken(
        ...
        try {
            ...
            ResponseCookie cookie = ResponseCookie.from("session", jwt)
                    .httpOnly(true)     // JS 拿不到，防 XSS
                    .secure(true)       // 只在 HTTPS
                    .sameSite("None")   // 允许跨站（OAuth 必须）
                    .path("/")          // 全站有效
                    .maxAge(Duration.ofHours(8)) // 过期时间可调
                    .build();
     ...
     */
    @PostMapping("/logout")
    public Result logout(ServerWebExchange exchange,
                         @RequestBody(required = false) RefreshTokenRequest request) {
