...
//            case SUBSCRIPTION_UPDATE -> handleSubscriptionUpdated(event);
    ...
    private void handleCheckoutCompleted(CreemWebhookEvent event) {
        ...
        log.info("📡 处理 checkout.completed: checkout={}", JSON.toJSONString(checkout));

        Order order = orderMapper.selectById(requestId);
        if (order == null) {
            log.warn("⚠️ 收到 Creem webhook 但本地查不到订单 requestId={}，可能是异常或绕过系统", requestId);
        } else {
            if (OrderStatus.COMPLETED.equals(order.getStatus())) {
                log.info("订单 {} 已处理过 checkout.completed，跳过", requestId);
                return;
            }
            updateAndSaveOrder(order, checkout);
        }
    }
    ...
    public void handleSubscriptionActiveAndTrialing(CreemWebhookEvent event) {
        ...
        try {
            ...
            User user = userMapper.findByEmail(email);
            if (user == null) {
                log.warn("未找到 email={} 的用户，跳过", email);
                return;
            }
     ...
     * 更新订单
     */
    private Order updateAndSaveOrder(Order order, CheckoutObject checkout) {
        order.setCheckoutId(checkout.getOrder().getId());
